...
def plot_k_and_get_best(estimator, cv, x, y, data_set_name, x_min, x_max, y_min, y_max):
    scoring = {'Accuracy': make_scorer(accuracy_score), 'AUC' : 'roc_auc'}
    
    gs = GridSearchCV(estimator, param_grid={'n_neighbors': range(x_min, x_max)}, scoring=scoring, cv=cv, 
    ...
    plt.title("K-Nearest Neighbors Performance vs K - " + data_set_name, fontsize=16)
    ...
    ax.set_xticks(range(x_min - 1, x_max + 1, 5))
...
def main():
    ...
    estimator = KNeighborsClassifier(weights = 'distance', p = 2)
    best_estimator = plot_k_and_get_best(estimator, cv, x, y, "Diabetic Retinopathy", 1, 100, 0.6, 1.01)

    title = "KNN (Diabetic Retinopathy)"
    ...
    estimator = KNeighborsClassifier(weights = 'distance', p = 2)
    best_estimator = plot_k_and_get_best(estimator, cv, x, y, "Digits", 1, 100, 0.9, 1.01)
